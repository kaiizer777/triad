...
- [ ] 4.1 — Orchestrator receives the task first whenever `current_mode ==
      orchestrator` (the default)
- [ ] 4.2 — Orchestrator **always states its routing reasoning out loud** in
...
- [ ] 4.3 — **Auto-proceed, no human confirmation needed**, only at the two
      extremes:
...
- [ ] 4.4 — **For everything in the genuine middle** → Orchestrator **must
...
- [ ] 4.5 — Log every routing decision (auto or confirmed) as its own
...
- [ ] 4.6 — Wire the clarify step from Phase 3 in *before* Orchestrator's
      routing judgment — clarify ambiguity first, then route, not the other
      way around
- [ ] 4.7 — **Test:** give Orchestrator a clearly trivial task, confirm it
      auto-routes to General Chat with a logged, stated reason
- [ ] 4.8 — **Test:** give Orchestrator a clearly critical task (e.g.
      something matching the hook blocklist), confirm it auto-routes to
      Triad with a logged, stated reason
- [ ] 4.9 — **Test:** give Orchestrator a genuinely ambiguous-complexity
      task, confirm it stops and asks for confirmation rather than silently
      picking
- [ ] 4.10 — **Test:** confirm every routing decision (all three cases
...
- [ ] 6.4 — Once spawned, the mini-Coder and mini-Reviewer run **their own
...
- [ ] 6.5 — **The mini-Reviewer preserves the core invariant**: no tool
...
- [ ] 6.6 — Wire the Phase 3 clarify step in immediately after the twin
      pair receives Orchestrator's one-message handoff, before their own
      loop starts
- [ ] 6.7 — The twin pair's own auto-commit behavior (Workflow 2 §2) should
...
- [ ] 6.8 — **Recursion/nesting guard**: a twin subagent's mini-Coder must
...
- [ ] 6.9 — When the twin pair's Reviewer and Coder agree the task is
...
- [ ] 6.10 — Replace Phase 4's "route medium tasks to Triad as a stand-in"
      with real twin-subagent routing now that this construct exists
- [ ] 6.11 — **Test:** give Orchestrator a genuinely medium-complexity task,
...
- [ ] 6.12 — **Test:** confirm a twin subagent's mini-Reviewer genuinely has
...
- [ ] 6.13 — **Test:** confirm the depth guard — attempt to have a twin
      pair's mini-Coder call `spawn_subagent` or spawn a nested twin, confirm
      it's blocked
- [ ] 6.14 — **Hard cap on twin-subagent turns.** Add a turn/time cap on the
...
- [ ] 6.15 — **Log twin-subagent start, not just completion.** Append a
      ...
      is Phase 7
- [ ] 6.16 — **Test:** confirm the turn cap from 6.14 actually triggers on a
...
- [ ] 6.17 — **Test:** confirm the start-of-twin log entry from 6.15
